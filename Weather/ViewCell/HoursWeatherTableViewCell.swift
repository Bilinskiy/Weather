...
extension HoursWeatherTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    cell.labelWeatherTime.text = indexPath.row == 0 ? "HoursWeatherTableViewCell.labelWeatherTime".localizationString() : self.hourlyWeather?[indexPath.row].dt?.dateFormatter(dateFormat: ParametersNetworkRequest.timeFormat)
